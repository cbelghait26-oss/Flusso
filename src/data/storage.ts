...
const KEYS = {
  ...
  // Workout sessions (persisted training room sessions)
  WORKOUT_SESSIONS: () => `${currentUserId}:data:workoutSessions`,
};
...
// ── Workout sessions (Training Room history) ──────────────────────────────────
...
export async function saveWorkoutSession(session: WorkoutSession): Promise<void> {
  try {
    requireUserId();
    const cached = getMemoryJson<WorkoutSession[]>(KEYS.WORKOUT_SESSIONS());
    const sessions = cached ? [...cached] : [];
    sessions.push(session);
    setMemoryJson(KEYS.WORKOUT_SESSIONS(), sessions);
    await AsyncStorage.setItem(KEYS.WORKOUT_SESSIONS(), JSON.stringify(sessions));
    syncToCloud("workoutSessions", sessions).catch(() => {});
  } catch {}
}
...
export async function loadWorkoutSessions(): Promise<WorkoutSession[]> {
  try {
    ...
    const cached = getMemoryJson<WorkoutSession[]>(KEYS.WORKOUT_SESSIONS());
    if (cached !== null) return cached;
    const local = await AsyncStorage.getItem(KEYS.WORKOUT_SESSIONS());
    if (local) {
      const parsed = JSON.parse(local) as WorkoutSession[];
      setMemoryJson(KEYS.WORKOUT_SESSIONS(), parsed);
      return parsed;
    }
    const cloud = await loadFromCloud("workoutSessions");
    if (cloud) {
      const arr = Array.isArray(cloud) ? cloud : [];
      setMemoryJson(KEYS.WORKOUT_SESSIONS(), arr);
      await AsyncStorage.setItem(KEYS.WORKOUT_SESSIONS(), JSON.stringify(arr));
      return arr;
    }
    setMemoryJson(KEYS.WORKOUT_SESSIONS(), []);
    return [];
  } catch { return []; }
}
